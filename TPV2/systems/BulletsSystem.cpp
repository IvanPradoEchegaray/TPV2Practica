...
{
	//Bala
	Vector2D bVel = Vector2D(0.0f, -1.0f).rotate(manager_->getComponent<Transform>(manager_->getHandler<MainHandler>())->getRot()) * (vel.magnitude() + 5.0f);
	Vector2D bPos = pos + Vector2D(width / 2.0f, height / 2.0f) - Vector2D(0.0f, height / 2.0f + 5.0f + 12.0f).rotate(manager_->getComponent<Transform>(manager_->getHandler<MainHandler>())->getRot())- Vector2D(2.0,10.0f);
	auto* bala = manager_->addEntity();
	manager_->addComponent<Transform>(bala,bPos, bVel, 5.0f, 20.f, manager_->getComponent<Transform>(manager_->getHandler<MainHandler>())->getRot());
